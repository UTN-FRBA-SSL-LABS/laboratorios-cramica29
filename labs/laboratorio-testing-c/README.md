...
> R: Inicializa la variable c (carrito) en 0, debido a que de otra forma, arrancaría con un valor basura.
---
...
> R: El test falló. Esperaba el valor 700 y obtuvo el valor 350 
...
> R: El bug está en la línea 24 -> total += c->items[i].precio;  /* BUG: falta multiplicar por cantidad */

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: La línea deberia multiplicar el precio unitario por la cantidad de ese producto antes de sumarlo al total. 
...
> R:  if (c->cantidad <= MAX_ITEMS) {   /* BUG: deberia ser < MAX_ITEMS */
...
Esas dos líneas no se ejecutaron al no ser testeadas
...
> R: Un test unitario se encarga de testear una función particular. Un test de integracion se encarga de testear la interacción, ejecución simultánea de múltiples funciones. El test unitario fue quien detectó el bug de carrito total. 
...
> R: C no tiene verificación de límites. Mientras que Python y Java, chequean de forma automática los índices en cada acceso, y lanza una excepción. 
...
> R: Es más difícil detectar errores leyendo código, debido a que algunos son casi imperceptibles, por ejemplo, la diferencia entre < y <=. Escribir los test define un resultado esperado, y no tiene el sesgo que tendría una persona corrigiendo. 
...
> R: El bug era que carrito_total sumaba solo precio en vez de precio * cantidad. Cuando cantidad = 1, precio * 1 es matemáticamente igual a precio, el resultado coincidía por casualidad con el resultado correcto. Con la cantidad = 2, se revela que la multiplicación por cantidad faltaba. .
